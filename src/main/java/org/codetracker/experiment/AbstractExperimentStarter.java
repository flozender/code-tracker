...
public abstract class AbstractExperimentStarter {
  ...
  protected static final String FOLDER_TO_CLONE = "tmp/";
  protected static final String SUMMARY_RESULT_FILE_NAME_FORMAT =
    "experiments/tracking-accuracy/%s/%s/summary-%s-%s.csv";
  protected static final String SUMMARY_RESULT_HEADER;
  protected static final String DETAILED_RESULT_HEADER =
  ...
  protected static final String TIME_RESULT_FILE_NAME_FORMAT =
    "experiments/tracking-accuracy/%s/%s/time-%s-%s.csv";
  protected static final String FINAL_RESULT_HEADER =
    "tool,oracle,level,processing_time_avg,processing_time_median,tp,fp,fn,precision,recall" +
    System.lineSeparator();
  protected static final String DETAILED_CONTENT_FORMAT =
  ...
  protected static final String ERROR_FILE_NAME_FORMAT =
    "experiments/tracking-accuracy/%s/%s/error-%s-%s-%s.txt";
  protected static final ObjectMapper MAPPER = new ObjectMapper();
  private static final String RESULT_FINAL_CSV =
    "experiments/tracking-accuracy/%s/%s/final.csv";
  private static final String PROCESSED_FILE_NAME_FORMAT =
    "experiments/tracking-accuracy/%s/%s/processed-%s-%s.csv";

  static {
    StringBuilder header = new StringBuilder();
    header.append(
      "instance,processing_time,analysed_commits,git_log_command_calls,step2,step3,step4,step5,"
    );
    for (Change.Type changeType : Change.Type.values()) {
      if (
        Change.Type.NO_CHANGE.equals(changeType) ||
        Change.Type.MULTI_CHANGE.equals(changeType) ||
        Change.Type.REMOVED.equals(changeType)
      ) continue;
      header
        .append("tp_")
        .append(changeType.getTitle().toLowerCase().replace(" ", "_"))
        .append(",");
      header
        .append("fp_")
        .append(changeType.getTitle().toLowerCase().replace(" ", "_"))
  ...
  protected static List<String[]> readResults(String fileName)
  ...
  protected static String getChangeKey(
  ...
  protected static HashMap<String, ChangeHistory> oracle(
    List<ChangeHistory> expectedChanges
  ) {
  ...
  protected static <
    T extends CodeElement
  > HashMap<String, ChangeHistory> processHistory(HistoryImpl<T> historyImpl) {
    HashMap<String, ChangeHistory> historyChanges = new HashMap<>();
    if (historyImpl.getGraph() == null) return historyChanges;
    ...
    for (EndpointPair<T> edge : edges) {
      ...
      for (Change change : changeList) {
        if (Change.Type.NO_CHANGE.equals(change.getType())) continue;
        ...
        historyChanges.put(
          getChangeKey(change.getType(), commitId),
          changeHistory
        );
      }
    }
  ...
  protected Set<String> getAllProcessedSamples(String oracleName)
    throws IOException {
    String processedFilePath = getProcessedFilePath(oracleName);
    Path path = writeToFile(
      processedFilePath,
      "file-names" + System.lineSeparator(),
      null,
  ...
  protected void calculateFinalResults(String oracleName) {
    ...
    try {
      ...
      List<String[]> summaryResults = readResults(
        getSummaryResultFileName(oracleName)
      );
      int tp = 0, fp = 0, fn = 0;
      ...
      double processingTimeAverage = processingTime
        .stream()
        .mapToInt(Integer::intValue)
        .average()
        .getAsDouble();
      writeToFile(
        String.format(RESULT_FINAL_CSV, getCodeElementName(), getToolName()),
        FINAL_RESULT_HEADER,
        String.format(
          FINAL_RESULT_FORMAT,
          getToolName(),
      ...
      List<String[]> detailedResults = readResults(
        getDetailedResultFileName(oracleName)
      );
      ...
      writeToFile(
        ...
        String.format(
          ...
          sumFP,
          sumFn,
          ((double) sumTp / (sumTp + sumFP)) * 100,
          ((double) sumTp / (sumTp + sumFn)) * 100
        ),
        StandardOpenOption.APPEND
      );
  ...
  private String getSummaryResultFileName(String oracleName) {
    return String.format(
      SUMMARY_RESULT_FILE_NAME_FORMAT,
      getCodeElementName(),
      getToolName(),
      getToolName(),
      oracleName
  ...
  protected void writeToSummaryFile(String oracleName, String content)
    throws IOException {
    writeToFile(
      getSummaryResultFileName(oracleName),
      SUMMARY_RESULT_HEADER,
      content,
      StandardOpenOption.APPEND
    );
  }
  ...
  private String getDetailedResultFileName(String oracleName) {
    return String.format(
      DETAILED_RESULT_FILE_NAME_FORMAT,
      getCodeElementName(),
      getToolName(),
      getToolName(),
      oracleName
  ...
  private String getTimeResultFileName(String oracleName) {
    return String.format(
      TIME_RESULT_FILE_NAME_FORMAT,
      getCodeElementName(),
      getToolName(),
      getToolName(),
      oracleName
  ...
  protected void writeToDetailedFile(
    String oracleName,
  ...
  protected void writeToTimeFile(
    String oracleName,
    String oracleFileName,
    String commitId,
    long processingTime,
  ...
  private String getTimeResultContent(
    String oracleFileName,
    String commitId,
    long processingTime,
    boolean move
  ...
  protected <
    H extends AbstractHistoryInfo, E extends CodeElement
  > void codeTracker(
    AbstractOracle<H> oracle,
    CheckedBiFunction<H, Repository, History<E>> tracker
  ) throws IOException {
    ...
    for (Map.Entry<String, H> oracleInstance : oracle.getOracle().entrySet()) {
      ...
      String repositoryName = repositoryWebURL
      ...
      try (
        ...
        HashMap<String, ChangeHistory> oracleChanges = oracle(
          historyInfo.getExpectedChanges()
        );
        ...
        long refactoringMinerProcessingTime =
          (System.nanoTime() - startTime) / 1000000;

        HashMap<String, ChangeHistory> detectedChanges = new HashMap<>();
        HashMap<String, ChangeHistory> notDetectedChanges = new HashMap<>(
          oracleChanges
        );
        HashMap<String, ChangeHistory> falseDetectedChanges = processHistory(
          (HistoryImpl<E>) history
        );
        ...
        content
          .append("\"")
          .append(historyInfo.getElementKey())
          .append("\",")
          .append(refactoringMinerProcessingTime)
          .append(",")
          .append(historyReport.getAnalysedCommits())
          .append(",")
          .append(historyReport.getGitLogCommandCalls())
          .append(",")
          .append(historyReport.getStep2())
          .append(",")
          .append(historyReport.getStep3())
          .append(",")
          .append(historyReport.getStep4())
          .append(",")
          .append(historyReport.getStep5())
          .append(",");
        ...
        for (Change.Type changeType : Change.Type.values()) {
          ...
          long tp = detectedChanges
            .values()
            .stream()
            .filter(changeHistory ->
              changeHistory.getChangeType().equals(changeType.getTitle())
            )
            .count();
          long fn = notDetectedChanges
            .values()
            .stream()
            .filter(historyResult ->
              historyResult.getChangeType().equals(changeType.getTitle())
            )
            .count();
          long fp = falseDetectedChanges
            .values()
            .stream()
            .filter(historyResult ->
              historyResult.getChangeType().equals(changeType.getTitle())
        ...
        List<ChangeHistory> historyResults = allChanges
          .values()
          .stream()
          .sorted(
            Comparator
              .comparing(ChangeHistory::getCommitTime)
              .reversed()
              .thenComparing(ChangeHistory::getCommitId)
          )
          .collect(Collectors.toList());
        for (ChangeHistory changeHistory : historyResults) {
          String changeKey = getChangeKey(changeHistory);
          String resultType;
          if (detectedChanges.containsKey(changeKey)) resultType =
            "TP"; else if (
            notDetectedChanges.containsKey(changeKey)
          ) resultType = "FN"; else if (
            falseDetectedChanges.containsKey(changeKey)
          ) resultType = "FP"; else resultType = "UN!";
          if (resultType == "FN") {
            continue;
          }
          ...
          writeToDetailedFile(
            oracleName,
            fileName,
            historyInfo.getRepositoryWebURL(),
            historyInfo.getElementKey(),
        ...
        writeToFile(
          getProcessedFilePath(oracleName),
          "file_name" + System.lineSeparator(),
